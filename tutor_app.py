...
# Real DA mode: implicit → semi-implicit → explicit
...
# ========== 1. 你的 Sentence Optimizer Prompt（终点生成器） ==========
# 👉 把这里替换成你已经在 DeepSeek 里调好的那一大段优化句法复杂度的 prompt
...
• Clause-level elaboration, which deepens hierarchical structure by adding or embedding clauses.
• Phrase-level elaboration, which increases informational density through nominalization, noun-phrase expansion, or prepositional modification.
...
1. Subordination – Add or embed a dependent clause (noun, relative, adverbial).
2. Embedding – Insert a phrase or clause within another.
3. Non-finite clauses – Use infinitives, participles, or gerunds for modification or complement.
4. Nominalization – Convert verbs/adjectives into noun forms to condense information.
5. Noun-phrase expansion – Extend noun phrases with modifiers or appositives.
6. Prepositional phrase expansion – Add prepositional structures to clarify relations.

Stylistic Constraints
• Use neutral, academic English.
• Avoid literary or conversational tone.
• Maintain roughly the same sentence length and structure.
• Output only the optimized sentence, without explanation.
"""
...
- If current_level == "implicit":
    - good_progress → "stay_implicit"
    - partial_progress → "stay_implicit"
    - no_progress → "upgrade_to_semi"

- If current_level == "semi-implicit":
    - good_progress → "reset_to_implicit"
    - partial_progress → "stay_semi"
    - no_progress → "upgrade_to_explicit"

- If current_level == "explicit":
    - good_progress → "reset_to_implicit"
    - partial_progress → "reset_to_implicit"
    - no_progress → "stay_explicit"
...
# ========== 5. MEDIATION_MAP: 六大 operation × 三层 DA 目标说明 ==========
MEDIATION_MAP = {
    "subordination": {
        "implicit":
    ...
    "embedding": {
        ...
        "semi-implicit":
            "Hint that academic writers sometimes place an idea inside a noun phrase, as in structures like 'the idea that…', but keep the language intuitive.",
        "explicit":
    ...
    "prepositional expansion": {
        "implicit":
            "Help the learner sense that the sentence might benefit from a gentle hint of context—for example time, place, or manner—without naming prepositional phrases.",
        "semi-implicit":
...
if st.session_state.get("chat_history"):
    ...
    for speaker, text in st.session_state.chat_history:
        ...
        # ---- 学生回合 ----
            ...
            st.markdown(
                f"""
                <div style='display:flex; align-items:flex-start; margin-bottom: 10px;'>
                    <div style="font-size:28px; margin-right:10px; margin-top:4px;">🙂</div>
                    <div style='line-height:1.4;'>
    ...
    st.markdown(
        """
        ### ✏️ What can SCAT help you with?

        SCAT helps you express the **same ideas** using clearer, smoother, and more academic sentence structures — without changing your meaning.
        ...
        No judgment, no pressure — only small nudges toward richer syntax. ❤️
        """
    )
    ...
    with col1:
        initial_sentence = st.text_area(
            "Your sentence (English / 中文 / mixed is okay):",
            value="",
            height=120
        )
    with col2:
        if st.button("🌱 Start / Reset SCAT cycle"):
            if initial_sentence.strip():
                ...
                with st.spinner("Thinking about a more complex but meaning-preserving version..."):
                    ...
                    # 首次 implicit 提示（learner_turn为空，因为是第一轮）
...
if st.session_state.get("chat_history"):
    ...
    if st.session_state.original_sentence:
        st.subheader("Step 2 · Reply to SCAT and see how the guidance grows")
            ...
            if learner_response.strip():
                ...
                with st.spinner("SCAT is reading your response and deciding the next hint..."):
                    ...
                    if decision == "upgrade_to_semi":
                        ...
                        # 进入新的 implicit cycle，以当前学生句子作为新 original
                ...
                st.warning("Tell SCAT something first — a revision, a question, or even 'I don't know' is okay.")

    # ===== Teacher / Researcher view =====
    with st.expander("👩‍🏫 Teacher view (for you as researcher / teacher)", expanded=False):
